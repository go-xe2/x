...
const MinUnixDatetime = -62135596800

// 使用自定义日期格式格式化输出日期。
func (t *Time) Format(format string) string {
	if t.Unix() == MinUnixDatetime {
		// 修正001-01-01日期
		return ""
	}
	runes := []rune(format)
...
func formatToRegexPattern(format string) string {
	s := xregex.Quote(formatToStdLayout(format))
	s, _ = xregex.ReplaceString(`[0-9]`, `[0-9]`, s)
	s, _ = xregex.ReplaceString(`[A-Za-z]`, `[A-Za-z]`, s)
	return s
}

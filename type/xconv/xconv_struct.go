...
import (
	"errors"
	"fmt"
	"github.com/go-xe2/x/core/structs"
	"github.com/go-xe2/x/type/xstring"
	"reflect"
	"strings"
)

var structTagPriority = []string{"json", "c"}
...
func Struct(params interface{}, pointer interface{}, mapping ...map[string]string) error {
	...
	tagMap, err := structs.TagMapName(pointer, structTagPriority, true)
	if err != nil {
		return err
	}
	...
	for i := 0; i < elem.NumField(); i++ {
		...
		if !xstring.IsLetterUpper(elemType.Field(i).Name[0]) {
			continue
		}
	...
	for mapK, mapV := range paramsMap {
		...
		for _, checkName := range []string{
			xstring.UcFirst(mapK),
			xstring.ReplaceByMap(mapK, map[string]string{
				"_": "",
				"-": "",
				" ": "",
			})} {
	...
	if err := Struct(params, pointer, mapping...); err != nil {
		...
		case reflect.Struct:
			...
			for i := 0; i < rv.NumField(); i++ {
				// Only do converting to public attributes.
				if !xstring.IsLetterUpper(rt.Field(i).Name[0]) {
					continue
				}
				trv := rv.Field(i)
				switch trv.Kind() {

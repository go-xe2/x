...
const (
	...
	mRECV_ALL_WAIT_TIMEOUT = time.Millisecond
)

// 创建TCP链接
func NewConn(addr string, timeout ...int) (*TConn, error) {
...
// 创建支持TLS加密通信的TCP链接
func NewConnTLS(addr string, tlsConfig *tls.Config) (*TConn, error) {
...
func NewConnKeyCrt(addr, crtFile, keyFile string) (*TConn, error) {
...
func NewConnByNetConn(conn net.Conn) *TConn {
	return &TConn{
		Conn:           conn,
		reader:         bufio.NewReader(conn),
		recvDeadline:   time.Time{},
		sendDeadline:   time.Time{},
		recvBufferWait: mRECV_ALL_WAIT_TIMEOUT,
	}
}

// 发送数据
func (c *TConn) Send(data []byte, retry ...TRetry) error {
	for {
		if _, err := c.Write(data); err != nil {
			...
			if len(retry) > 0 {
				...
				if retry[0].Interval == 0 {
					retry[0].Interval = mDEFAULT_RETRY_INTERVAL
				}
...
func (c *TConn) Recv(length int, retry ...TRetry) ([]byte, error) {
	var err error       // 读取错误
	var size int        // 读取长度
	var index int       // 已读取长度
	...
	if length > 0 {
		...
		buffer = make([]byte, mDEFAULT_READ_BUFFER_SIZE)
	}
	...
	for {
		...
		if size > 0 {
			...
			if length > 0 {
				...
				if index >= mDEFAULT_READ_BUFFER_SIZE {
					// 如果长度超过了自定义的读取缓冲区，那么自动增长
					buffer = append(buffer, make([]byte, mDEFAULT_READ_BUFFER_SIZE)...)
				} else {
...
func (c *TConn) RecvLine(retry ...TRetry) ([]byte, error) {
	var err error
	var buffer []byte
	data := make([]byte, 0)
	for {
...
// 带超时时间的数据获取
func (c *TConn) RecvWithTimeout(length int, timeout time.Duration, retry ...TRetry) (data []byte, err error) {
...
// 带超时时间的数据发送
func (c *TConn) SendWithTimeout(data []byte, timeout time.Duration, retry ...TRetry) (err error) {
...
// 发送数据并等待接收返回数据
func (c *TConn) SendRecv(data []byte, receive int, retry ...TRetry) ([]byte, error) {
...
func (c *TConn) SendRecvWithTimeout(data []byte, receive int, timeout time.Duration, retry ...TRetry) ([]byte, error) {
	if err := c.Send(data, retry...); err == nil {
...
func (c *TConn) SetDeadline(t time.Time) error {
...
func (c *TConn) SetRecvDeadline(t time.Time) error {
...
func (c *TConn) SetSendDeadline(t time.Time) error {
...
func (c *TConn) SetRecvBufferWait(bufferWaitDuration time.Duration) {

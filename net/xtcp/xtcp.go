...
const (
	mDEFAULT_RETRY_INTERVAL   = 100 // (毫秒)默认重试时间间隔
	mDEFAULT_READ_BUFFER_SIZE = 128 // (byte)默认数据读取缓冲区大小
)
...
// (面向短链接)发送数据
func Send(addr string, data []byte, retry ...TRetry) error {
...
// (面向短链接)发送数据并等待接收返回数据
func SendRecv(addr string, data []byte, receive int, retry ...TRetry) ([]byte, error) {
	conn, err := NewConn(addr)
	if err != nil {
		return nil, err
	}
...
// (面向短链接)带超时时间的数据发送
func SendWithTimeout(addr string, data []byte, timeout time.Duration, retry ...TRetry) error {
	conn, err := NewConn(addr)
	if err != nil {
		return err
	}
...
func SendRecvWithTimeout(addr string, data []byte, receive int, timeout time.Duration, retry ...TRetry) ([]byte, error) {
	conn, err := NewConn(addr)
	if err != nil {
		return nil, err
	}
...
func LoadKeyCrt(crtFile, keyFile string) (*tls.Config, error) {
	crtPath, err := xfile.Search(crtFile)
	if err != nil {
		return nil, err
	}
	keyPath, err := xfile.Search(keyFile)
	if err != nil {
		return nil, err
	}

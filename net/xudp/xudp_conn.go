...
// 创建TCP链接
func NewConn(raddr string, laddr ...string) (*TConn, error) {
...
// 将*net.UDPTConn对象转换为*TConn对象
func NewConnByNetConn(udp *net.UDPConn) *TConn {
	return &TConn{
		UDPConn:        udp,
...
// 发送数据
func (c *TConn) Send(data []byte, retry ...TRetry) (err error) {
	for {
		if c.raddr != nil {
...
func (c *TConn) SetRecvBufferWait(d time.Duration) {
	c.recvBufferWait = d
}

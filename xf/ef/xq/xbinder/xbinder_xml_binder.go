...
func (bd *TDbQueryXmlBinder) NewInstance(options ...map[string]interface{}) DbQueryBinder {
	if len(options) > 0 && options[0] != nil {
		option := options[0]
		return NewQryXmlBinder(nil).SetOptions(option)
	}
	return bd
}
...
func (bd *TDbQueryXmlBinder) buildConvertBuf(colInfos ...*TQueryColInfo) {
...
func (bd *TDbQueryXmlBinder) BuildRow(row int, colInfos *[]QueryColValue) (result interface{}, exit bool) {
	for i := 0; i < bd.colCount; i++ {
		...
		bd.row += "\n"
		bd.row += xxml.MakeNode(fdName, v, 2)
	}
	return xxml.MakeNode("row", bd.row, 1), false
}

// 行创建完成
// rowData为BuildRow所创建的行数据
func (bd *TDbQueryXmlBinder) EndBuildRow(rowData interface{}) {
	if item, ok := rowData.(xxml.XmlStr); ok {
		bd.data += "\n"
		bd.data += item
	}
}
...
func (bd *TDbQueryXmlBinder) EndBuild() interface{} {
	...
	return xxml.MakeNode("rows", bd.data, 0)
}

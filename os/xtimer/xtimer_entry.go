...
type TEntry struct {
	...
	singleton     *_type.TBool // 任务是否单例运行
	status        *_type.TInt  // 任务状态(0: ready;  1: running; 2: stopped; -1: closed), 层级entry共享状态
	times         *_type.TInt  // 还需运行次数
...
func (w *twheel) addEntry(interval time.Duration, job JobFunc, singleton bool, times int, status int) *TEntry {
	if times <= 0 {
		times = mDEFAULT_TIMES
	}
	...
	entry := &TEntry{
		wheel:         w,
		job:           job,
		times:         _type.NewInt(times),
		status:        _type.NewInt(status),
		create:        ticks,
		interval:      num,
		singleton:     _type.NewBool(singleton),
		createMs:      nowMs,
		intervalMs:    ms,
		rawIntervalMs: ms,
	}
...
func (w *twheel) addEntryByParent(interval int64, parent *TEntry) *TEntry {
...
// 检测当前任务是否可运行。
func (entry *TEntry) check(nowTicks int64, nowMs int64) (runnable, addable bool) {
	...
	if diff := nowTicks - entry.create; diff > 0 && diff%entry.interval == 0 {
		...
		if times < 2000000000 && times > 1000000000 {
			entry.times.Set(mDEFAULT_TIMES)
		}
		return true, true
	}
	return false, true

...
const (
	mPROC_ENV_KEY_PPID_KEY = "XPROC_PPID"
	mPROC_TEMP_DIR_ENV_KEY = "XPROC_TEMP_DIR"
)
...
func PPid() int {
	...
	// mPROC_ENV_KEY_PPID_KEY为gproc包自定义的父进程
	ppidValue := os.Getenv(mPROC_ENV_KEY_PPID_KEY)
	if ppidValue != "" && ppidValue != "0" {
		return t.Int(ppidValue)
	}
	return PPidOS()
}
...
func SetPPid(ppid int) error {
	if ppid > 0 {
		return os.Setenv(mPROC_ENV_KEY_PPID_KEY, t.String(ppid))
	} else {
		return os.Unsetenv(mPROC_ENV_KEY_PPID_KEY)
	}
}

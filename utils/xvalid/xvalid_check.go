...
import (
	...
	"github.com/go-xe2/x/net/xipv4"
	"github.com/go-xe2/x/net/xipv6"
...
const (
	mSINGLE_RULE_PATTERN = `^([\w-]+):{0,1}(.*)` // 单条规则匹配正则
)

var (
	// 默认错误消息管理对象(并发安全)
	errorMsgMap = xsafeMap.NewStrStrMap()

	// 单规则正则对象，这里使用包内部变量存储，不需要多次解析
	ruleRegex, _ = regexp.Compile(mSINGLE_RULE_PATTERN)
...
func Check(value interface{}, rules string, msgs interface{}, params ...interface{}) *TValidError {
	...
	val := strings.TrimSpace(t.String(value))
	data := make(map[string]string)
	errorMsgs := make(map[string]string)
	if len(params) > 0 {
		for k, v := range t.Map(params[0]) {
			data[k] = t.String(v)
		}
	}
	// 自定义错误消息处理
	...
	default:
		for k, v := range t.Map(msgs) {
			customMsgMap[k] = t.String(v)
		}
	}
	...
	for i := 0; ; {
		...
		if _, ok := allSupportedRules[array[0]]; !ok {
			if i > 0 && ruleItems[i-1][:5] == "regex" {
				...
				return newValidErrorStr("parse_error", "invalid rules:"+rules)
			}
		} else {
			i++
		}
	...
	for index := 0; index < len(ruleItems); {
		...
		case "regex":
			...
			for i := index + 1; i < len(ruleItems); i++ {
				...
				if !xregex.IsMatchString(mSINGLE_RULE_PATTERN, ruleItems[i]) {
					ruleVal += "|" + ruleItems[i]
					index++
				}
			}
			match = xregex.IsMatchString(ruleVal, val)

		// 日期格式，
		case "date":
			// 使用标准日期格式检查，但是日期之间必须带连接符号
			if _, err := xtime.StrToTime(val); err == nil {
				match = true
				break
			}
			// 检查是否不带日期连接符号的格式
			if _, err := xtime.StrToTime(val, "Ymd"); err == nil {
				match = true
				break
			}
		...
		// 日期格式，需要给定日期格式
		case "date-format":
			if _, err := xtime.StrToTimeFormat(val, ruleVal); err == nil {
				match = true
			}
		 ...
		 */
		case "phone":
			match = xregex.IsMatchString(`^13[\d]{9}$|^14[5,7]{1}\d{8}$|^15[^4]{1}\d{8}$|^16[\d]{9}$|^17[0,3,5,6,7,8]{1}\d{8}$|^18[\d]{9}$|^19[\d]{9}$`, val)

		// 国内座机电话号码："XXXX-XXXXXXX"、"XXXX-XXXXXXXX"、"XXX-XXXXXXX"、"XXX-XXXXXXXX"、"XXXXXXX"、"XXXXXXXX"
		case "telephone":
			match = xregex.IsMatchString(`^((\d{3,4})|\d{3,4}-)?\d{7,8}$`, val)

		// 腾讯QQ号，从10000开始
		case "qq":
			match = xregex.IsMatchString(`^[1-9][0-9]{4,}$`, val)

		// 中国邮政编码
		case "postcode":
			match = xregex.IsMatchString(`^\d{6}$`, val)
		...
		*/
		case "id-number":
			match = xregex.IsMatchString(`(^[1-9]\d{5}(18|19|([23]\d))\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{3}[0-9Xx]$)|(^[1-9]\d{5}\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{3}$)`, val)

		// 通用帐号规则(字母开头，只能包含字母、数字和下划线，长度在6~18之间)
		case "passport":
			match = xregex.IsMatchString(`^[a-zA-Z]{1}\w{5,17}$`, val)

		// 通用密码(任意可见字符，长度在6~18之间)
		case "password":
			match = xregex.IsMatchString(`^[\w\S]{6,18}$`, val)

		// 中等强度密码(在弱密码的基础上，必须包含大小写字母和数字)
		case "password2":
			if xregex.IsMatchString(`^[\w\S]{6,18}$`, val) && xregex.IsMatchString(`[a-z]+`, val) && xregex.IsMatchString(`[A-Z]+`, val) && xregex.IsMatchString(`\d+`, val) {
				match = true
			}

		// 强等强度密码(在弱密码的基础上，必须包含大小写字母、数字和特殊字符)
		case "password3":
			if xregex.IsMatchString(`^[\w\S]{6,18}$`, val) && xregex.IsMatchString(`[a-z]+`, val) && xregex.IsMatchString(`[A-Z]+`, val) && xregex.IsMatchString(`\d+`, val) && xregex.IsMatchString(`[^a-zA-Z0-9]+`, val) {
				match = true
			}

		// json
		case "json":
			if _, err := xjson.Decode([]byte(val)); err == nil {
				match = true
			}
		...
		// 邮件
		case "email":
			match = xregex.IsMatchString(`^[a-zA-Z0-9_\-\.]+@[a-zA-Z0-9_\-]+(\.[a-zA-Z0-9_\-]+)+$`, val)

		// URL
		case "url":
			match = xregex.IsMatchString(`(https?|ftp|file)://[-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|]`, val)

		// domain
		case "domain":
			match = xregex.IsMatchString(`^([0-9a-zA-Z][0-9a-zA-Z-]{0,62}\.)+([0-9a-zA-Z][0-9a-zA-Z-]{0,62})\.?$`, val)

		// IP(IPv4/IPv6)
		case "ip":
			match = xipv4.Validate(val) || xipv6.Validate(val)

		// IPv4
		case "ipv4":
			match = xipv4.Validate(val)

		// IPv6
		case "ipv6":
			match = xipv6.Validate(val)

		// MAC地址
		case "mac":
			match = xregex.IsMatchString(`^([0-9A-Fa-f]{2}[\-:]){5}[0-9A-Fa-f]{2}$`, val)
	...
	if len(errorMsgs) > 0 {
		return newValidError([]string{rules}, TValidErrorMap{
			// 单条数值校验没有键名
			"": errorMsgs,
		})
	}

...
var (
	// 同时支持valid和v标签,xvalid,gvalid，优先使用valid
	structTagPriority = []string{"xvalid", "valid", "v", "gvalid"}
)
...
func CheckStruct(object interface{}, rules interface{}, msgs ...CustomMsg) *TValidError {
	...
	fields, err := structs.MapField(object, structTagPriority, true)
	if err != nil {
		panic(err)
	}
	for nameOrTag, field := range fields {
	...
	if len(errorMaps) > 0 {
		return newValidError(errorRules, errorMaps)
	}
	return nil
}

...
type TValidError struct {
	...
	errors    TValidErrorMap    // 完整的数据校验结果存储(map无序)
...
// 创建一个校验错误对象指针(校验错误)
func newValidError(rules []string, errors map[string]map[string]string) *TValidError {
	return &TValidError{
		rules:  rules,
		errors: errors,
	}
...
// 创建一个校验错误对象指针(内部错误)
func newValidErrorStr(key, err string) *TValidError {
	return &TValidError{
		rules: nil,
